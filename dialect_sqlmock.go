...
var (
	sqlmockReservedWords = map[string]bool{
		"ADD":               true,
		"ALL":               true,
		"ALTER":             true,
		"ANALYZE":           true,
...
func (db *sqlmock) Init(d *core.DB, uri *core.Uri, drivername, dataSourceName string) error {
	return db.Base.Init(d, db, uri, drivername, dataSourceName)
}

func (db *sqlmock) SetParams(params map[string]string) {
...
func (db *sqlmock) SqlType(c *core.Column) string {
	var res string
...
func (db *sqlmock) SupportInsertMany() bool {
	return true
}

func (db *sqlmock) IsReserved(name string) bool {
	_, ok := sqlmockReservedWords[name]
	return ok
}

func (db *sqlmock) Quote(name string) string {
	return "`" + name + "`"
}

func (db *sqlmock) QuoteStr() string {
	return "`"
}

func (db *sqlmock) SupportEngine() bool {
	return true
}

func (db *sqlmock) AutoIncrStr() string {
	return "AUTO_INCREMENT"
}

func (db *sqlmock) SupportCharset() bool {
	return true
}

func (db *sqlmock) IndexOnTable() bool {
	return true
}

func (db *sqlmock) IndexCheckSql(tableName, idxName string) (string, []interface{}) {
...
/*func (db *sqlmock) ColumnCheckSql(tableName, colName string) (string, []interface{}) {
	args := []interface{}{db.DbName, tableName, colName}
	sql := "SELECT `COLUMN_NAME` FROM `INFORMATION_SCHEMA`.`COLUMNS` WHERE `TABLE_SCHEMA` = ? AND `TABLE_NAME` = ? AND `COLUMN_NAME` = ?"
	return sql, args
}*/

func (db *sqlmock) TableCheckSql(tableName string) (string, []interface{}) {
	args := []interface{}{db.DbName, tableName}
	sql := "SELECT `TABLE_NAME` from `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA`=? and `TABLE_NAME`=?"
	return sql, args
}

func (db *sqlmock) GetColumns(tableName string) ([]string, map[string]*core.Column, error) {
	args := []interface{}{db.DbName, tableName}
...
func (db *sqlmock) GetTables() ([]*core.Table, error) {
...
func (db *sqlmock) GetIndexes(tableName string) (map[string]*core.Index, error) {
	args := []interface{}{db.DbName, tableName}
...
func (db *sqlmock) CreateIndexSql(tableName string, index *core.Index) string {
	quote := db.Quote
...
func (db *sqlmock) CreateTableSql(table *core.Table, tableName, storeEngine, charset string) string {
	var sql string
...
func (db *sqlmock) Filters() []core.Filter {
...
func (p *sqlmockDriver) Parse(driverName, dataSourceName string) (*core.Uri, error) {
	...
	uri := &core.Uri{DbType: core.SQLMOCK}
